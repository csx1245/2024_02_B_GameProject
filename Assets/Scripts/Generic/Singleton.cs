...
public abstract class Singleton<T> : MonoBehaviour where T : MonoBehaviour
{
    ...
    public static T Instance
    {
        get
        {
            ...
            if (isQuitting)
            {
                Debug.Log($"[싱글톤] '{typeof(T)}' 인스턴스가 애플리케이션 종료 중에 접근되고 있습니다. 고스트 객체 방지를 위해 null 반환");
                return null;
            }
    ...
    protected virtual void Awake()
    {
        ...
        {
            Debug.LogWarning($"[싱글톤] {typeof(T)}의 인스턴스가 이미 존재합니다. 이 중복을 파괴합니다.");
            Destroy(gameObject);
        }
    }

    //OnApplicationQuit 메서드 종료 플래그를 설정하는 데 사용
    protected virtual void OnApplicationQuit()
    {
        isQuitting = true;
    }
    //OnDestroy 메서드 예상치 못한 파괴를 체크하는 데 사용
